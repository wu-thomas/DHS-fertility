# Urban Fraction for Stratified Model
...
Stratified model produces estimates separately for urban and rural. An aggregation step is needed for obtaining overall estimates. As summarized in the following equation, the beta-binomial model generates strata specific U5MR estimates and we need the urban/rural proportion for the female population $q_{i,t}$ and $1-q_{i,t}$ for all region $i$ and time $t$ to obtain the final overall estimates. 

$$p_{i,t} = q_{i,t} \times Fert_{i,t,R} + (1-q_{i,t} ) \times Fert{i,t,U} $$

The complete algorithm of finding those proportions is implemented in two scripts: prepare_thresh.R, thresh.R and they should be run in the order listed. This vignette focuses on finding and downloading the data anda S highlights some of the major steps.
...
#### 100m $\times$ 100m raster for female 5-year age group population at the years for estimation

These rasters are used for aggregating strata specific U5MR. We use worldpop population density broken down by age and sex (available at https://www.worldpop.org/geodata/listing?id=30). The automated downloading scripts can also be found within prepare_pop.R
